...
const mockRoom = {
  ...
  member_count: 4,
  account_email: "netflixshared@example.com",
  account_password: "SuperSecure123!",
  members: [
    {
      username: "john_doe",
      role: "owner",
...
const RoomDetailDemo: React.FC = () => {
  ...
  const paidCount = room.members.filter(member => member.payment_status === 'paid').length;
  const pendingCount = room.members.filter(member => member.payment_status === 'pending').length;
  const overdueCount = room.members.filter(member => member.payment_status === 'overdue').length;

  // Individual cost calculation
  const individualCost = parseFloat(room.cost) / room.member_count;
